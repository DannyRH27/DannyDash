...
function PreviousOrderIndexItem(props) {
  ...
  const { order } = props;
  console.log(JSON.parse(order.deliveryEta))
  const date = new Date();
  const month = date.getMonth();
  const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
  const day = date.getDate();
  const year = date.getFullYear();
  const strDate = `${months[month]} ${day} ${year}, ${JSON.parse(order.deliveryEta)[0]}  •  $${order.total}`;
  ...
  const IndexItem = new Date(JSON.parse(order.deliveryEta)[1]) < date ? (
